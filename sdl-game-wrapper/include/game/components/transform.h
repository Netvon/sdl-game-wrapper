#ifndef SGW_COMPONENTS_TRANSFORM_H
#define SGW_COMPONENTS_TRANSFORM_H
#include <glm/glm.hpp>
...
namespace sgw::components {
    ...
    struct generic_transform {
        ...
        explicit constexpr generic_transform(vector_type position)
            : m_position(position) {}
        constexpr generic_transform(value_type x, value_type y)
            : m_position(x, y) {}
        constexpr generic_transform(value_type x, value_type y, value_type rotation)
            : m_position(x, y)
            , m_rotation(rotation) {}
        constexpr generic_transform(vector_type position, value_type rotation)
            : m_position(position)
            , m_rotation(rotation) {}
        ...
        [[nodiscard]] constexpr const vector_type& get_position() const noexcept { return m_position; }
        ...
        [[nodiscard]] constexpr const value_type& get_rotation() const noexcept { return m_rotation; }

        constexpr void set_rotation(value_type rotation) noexcept { m_rotation = rotation; }

        constexpr void add_rotation(value_type rotation) noexcept { m_rotation += rotation; }

        constexpr void subtract_rotation(value_type rotation) noexcept { m_rotation -= rotation; }
...
#endif // SGW_COMPONENTS_TRANSFORM_H